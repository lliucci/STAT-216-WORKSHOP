...
![Importing Data](images/ImportingData.jpg)
...
![Select heading: YES](images/ImportHeading.jpg)
...
```{r Intro-To-R-Exercise-1-solution}
RollerCoasters[5:8, ]
```

```{r Intro-To-R-Exercise-1-check}
grade_code()
```
...
- You can also use **Tab-Completion** to reduce issues with case sensitivity. This is where `R` will suggest auto-complete options when you start typing in a variable name. By selecting one of these options, you are sure to reference something that `R` will recognize. Here is an example of what this looks like:

![TabComplete](images/tabcomplete.png)
$~$
...
```{r Server-Exercise-2-solution}
summary(RollerCoasters)
```

```{r Server-Exercise-2-check}
grade_code()
```
...
One of the most common issues that can come up when working with `ggplot` is data sets being misspelled or incorrectly typed. Let's take a look at what this looks like. Additionally, try to correct the error.
...
```{r ggplot-troubleshooting-1-solution, warning = F}
ggplot(data = RollerCoasters,
       mapping = aes(x = Design, y = Top_Speed)) + 
  geom_boxplot()
```

```{r ggplot-troubleshooting-1-check}
grade_code()
```
...
```{r ggplot-troubleshooting-2-solution, warning = F}
ggplot(data = RollerCoasters,
       mapping = aes(x = Year_Opened)) +
  geom_histogram()
```

```{r ggplot-troubleshooting-2-check}
grade_code()
```
...
```{r ggplot-troubleshooting-3-solution, warning = F}
ggplot(data = RollerCoasters,
       mapping = aes(x = Duration, y = Length)) +
  geom_point()
```

```{r ggplot-troubleshooting-3-check}
grade_code()
```
...
```{r ggplot-troubleshooting-4-solution, warning = F}
ggplot(data = RollerCoasters,
       mapping = aes(x = Duration, y = Max_Height)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
```

```{r ggplot-troubleshooting-4-check}
grade_code()
```
...
```{r catstats-exercise-1-solution}
one_proportion_test(
  probability_success = .50,
  sample_size = 10,
  summary_measure = "proportion",
...
**A**. Replacing the word, or 'call', that functions will use to reference the dataset instead of the word 'datasetname'. The actually name of the uploaded dataset, which will usually be read in from a text file or .csv file, will appear in the Environment and **must be typed exactly how it appears there** or else `R` will not recognize it.
...
**E**. Accidentally deleting quotation marks or commas after an argument within a function. If you are unsure if this has occurred, don't hesitate to ask the student to open another unedited version of the same file under a different name (so it doesn't override all of the edits and progress they have made on their current R file) to double check that area of the template code.
