...
---ADD NOTE ON HOW TO CHANGE THE FONT SIZE FOR ACCESIBILITY
...
In the Environment tab (top right), click _Import Dataset_ -> choose the _From Text (base)_ option 
![importdata](images/ImportingData.jpg)
-> students will select the dataset from where the file is downloaded on their device.

Once they have selected the dataset, another window will appear. Here, it is critical that they **select YES for Heading** for the dataset to be uploaded correctly and usable.
![headingYES](images/ImportHeading.jpg)
After 'Yes' is selected, click 'Import' and you are done! 

If a student claims they imported the dataset but it still is not reading in correctly for them, walk through this process with them to double check.
...
```{r practice3pt2, exercise = T}
2 + 2 #Look at how well R can add!
```

Comments do not need to be ran for the code to work. If comments are run, they do not effect the output. However, it is good practice to good documentation of your code through comments so others can easily understand it. Comments can exist on a line alone, after a line with a section of a function, but cannot exist between code on the same line.
...
![Library Packages](images/RInstallvsLibrary.jpeg)
...
First, it must be **saved to the server**. To do this, start in the top left section of the server. Make sure the student is on the tab of the file the want to save. In that top left section, click _File_ -> _Save As_ -> then students will be prompted to name their file -> select _Save_.

Now the file name should appear under the Files tab in the bottom right corner. Direct students to click the check box next to the file name -> then select _More_ -> click _Export_ -> _Download_. This process **saves the file from the server to their actual device**. Now students should have access to the file on their device so they can upload it to Gradescope if asked.
...
---- DOUBLE CHECK R RFILES TO SEE WHICH ARE MOST RELEVANT

For this example, lets create a scatter plot. We can do this by adding the `geom_point` argument to our graph. Note that for each layer that is added on, we must separate the lines with a plus (+).
...
Now, fill in the template below to create a plot with Duration on the x-axis and Maximum Height on the y-axis. (Hint: _replace_ the underscores with the data set name and variable names.)

```{r ggplot-troubleshooting-4, exercise = T, warning = F}
ggplot(data = ___,
...
--- IF A STUDENT DOES NOT TRY TO DOWNLOAD IT USING REMOTES AND TRYS TO DOWNLOAD IT USING PACKAGES, HERE IS THE ERROR THEY WILL SEE
...
-   "**variable**" : This one is pretty straight forward. When only dealing with one variable of interest, this is where the should put the name of the variable, *making sure to replicate how it appears in the dataset*. Pay attention to Capitalization and lowercase, underscores, and other potentially tricky formatting with variable names.
...
**D**. ***NOT Reading!*** Students are often in a hurry to complete to activity or lab for the day, and do not read the instructions as closely as they should. Thus, they may to skip over explicit instructions commented within the code and ask you instead. Many exercises will be presented to the students like this:
...
--- CAREFULLY CHOOSE YOUR WORDS TO NOT DISCOURAGE THEM AND STILL ENCOURAGE THEM TO WORK TOWARDS AN ANSWER. although we may understand coding now, this was also difficult for us at one time too. avoid using words such as 'easy' 'simple' and 'just do this...'
...
* If it finds _no_ __identical match__, it will tell you that "`No documentation for ____ in specified packages and libraries:,`" and suggests you use a `??` instead.  
    ...
    + A `??` in front of the function name will search __all of `R`__ for named functions similar to what you typed.  
...
More information about these MSU R Workshops can be found [here](https://www.montana.edu/datascience/training/).
